...
class BotService:
    def __init__(self, serv_ip, serv_port, serv_pass):
        ...
        if global_settings.cfg.get(C_MEDIA_SETTINGS, P_TEMP_MED_DIR, fallback=None):
            ...
            dir_utils.make_directory(
                f"{dir_utils.get_main_dir()}/cfg/temporary_media_directory"
            )
            global_settings.cfg[C_MEDIA_SETTINGS][P_TEMP_MED_DIR] = (
                f"{dir_utils.get_main_dir()}/cfg/temporary_media_directory"
            )
        if global_settings.cfg.get(C_MEDIA_SETTINGS, P_PERM_MEDIA_DIR, fallback=None):
            dir_utils.make_directory(
                global_settings.cfg[C_MEDIA_SETTINGS][P_PERM_MEDIA_DIR]
            )
        else:
            dir_utils.make_directory(
                f"{dir_utils.get_main_dir()}/cfg/permanent_media_directory"
            )
            global_settings.cfg[C_MEDIA_SETTINGS][P_PERM_MEDIA_DIR] = (
                f"{dir_utils.get_main_dir()}/cfg/permanent_media_directory"
            )
        ...
        log(
            INFO,
            "######### Initialized Temporary Directories",
            origin=L_STARTUP,
            print_mode=PrintMode.VERBOSE_PRINT.value,
        )
        ...
        if global_settings.safe_mode:
            ...
            log(
                INFO,
                "Initialized plugins with safe mode.",
                origin=L_STARTUP,
                print_mode=PrintMode.VERBOSE_PRINT.value,
            )
        else:
    ...
    def initialize_mumble(self, md: MumbleData):
        global_settings.mumble_inst = mumble.Mumble(
            md.ip_address,
            port=md.port,
            user=md.user_id,
        ...
        global_settings.core_callbacks.append_to_callback(
            CALLBACK.TEXT_MESSAGE_RECEIVED, self.message_received
        )
        # Callback - sound_received
        global_settings.mumble_inst.callbacks.set_callback(
            CALLBACK.SOUND_RECEIVED, global_settings.clbk_service.sound_received
        )
        global_settings.core_callbacks.append_to_callback(
            CALLBACK.SOUND_RECEIVED, self.sound_received
        )
        # Callback - on_connected
        global_settings.mumble_inst.callbacks.set_callback(
            CALLBACK.CONNECTED, global_settings.clbk_service.connected
        )
        global_settings.core_callbacks.append_to_callback(
            CALLBACK.CONNECTED, self.on_connected
        )
        # Callback - disconnected
        global_settings.mumble_inst.callbacks.set_callback(
            CALLBACK.DISCONNECTED, global_settings.clbk_service.disconnected
        )
        global_settings.core_callbacks.append_to_callback(
            CALLBACK.DISCONNECTED, self.on_disconnected
        )
        # Callback - user_created
        global_settings.mumble_inst.callbacks.set_callback(
            CALLBACK.USER_CREATED, global_settings.clbk_service.user_created
        )
        # Callback - user_updated
        global_settings.mumble_inst.callbacks.set_callback(
            CALLBACK.USER_UPDATED, global_settings.clbk_service.user_updated
        )
        # Callback - user_removed
        global_settings.mumble_inst.callbacks.set_callback(
            CALLBACK.USER_REMOVED, global_settings.clbk_service.user_removed
        )
        # Callback - channel_created
        global_settings.mumble_inst.callbacks.set_callback(
            CALLBACK.CHANNEL_CREATED, global_settings.clbk_service.channel_created
        )
        # Callback - channel_removed
        global_settings.mumble_inst.callbacks.set_callback(
            CALLBACK.CHANNEL_REMOVED, global_settings.clbk_service.channel_removed
        )
        # Callback - channel_updated
        global_settings.mumble_inst.callbacks.set_callback(
            CALLBACK.CHANNEL_UPDATED, global_settings.clbk_service.channel_updated
        )
                    ...
                    for x, sub_item in enumerate(alias_commands):
                        if not remote_cmd:
                            ...
                            sub_text = RemoteTextMessage(
                                channel_id=global_settings.mumble_inst.users.myself[
                                    "channel_id"
                                ],
                                session=global_settings.mumble_inst.users.myself[
                                    "session"
                                ],
                                message=text.message,
                                actor=global_settings.mumble_inst.users.myself[
                                    "session"
                                ],
                            )
                        if len(item.split()) > 1:
    ...
    def on_connected(self, data):
        log(
            INFO,
            f"{runtime_utils.get_bot_name()} is Online.",
            origin=L_STARTUP,
            print_mode=PrintMode.REG_PRINT.value,
        )
    ...
    def on_disconnected(self, data):
        log(
            INFO,
            f"{runtime_utils.get_bot_name()} has disconnected.",
            origin=L_SHUTDOWN,
            print_mode=PrintMode.REG_PRINT.value,
        )
    ...
    def sound_received(self, audio_data):
        ...
        if (
            audioop.rms(audio_chunk.pcm, 2)
            > global_settings.aud_interface.status["ducking_threshold"]
            and global_settings.aud_interface.status["duck_audio"]
        ):
            global_settings.aud_interface.audio_utilities.duck_volume()
            global_settings.aud_interface.status["duck_start"] = time()
            global_settings.aud_interface.status["duck_end"] = (
                time()
                + global_settings.aud_interface.audio_utilities.get_ducking_end_delay()
            )
        ...
        try:
            while not global_settings.exit_flag:
                if (
                    ...
                    and time() > global_settings.aud_interface.status["duck_end"]
                ):
            ...
            log(
                INFO,
                f"{runtime_utils.get_bot_name()} was booted offline by a keyboard interrupt (ctrl-c).",
                origin=L_SHUTDOWN,
                print_mode=PrintMode.VERBOSE_PRINT.value,
            )
