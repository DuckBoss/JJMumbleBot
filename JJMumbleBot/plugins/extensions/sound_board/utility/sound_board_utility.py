...
def download_clip(url, name, time_range=None, ffmpeg_path='ffmpeg', proxy=''):
    ...
    try:
        ...
        if time_range:
            ...
            subprocess.call([ffmpeg_path, "-nostats", "-hide_banner", "-ss", time_range[0], "-i", info_dict['url'], "-to", end_range, "-c", "copy", f"{dir_utils.get_perm_med_dir()}/{settings.plugin_name}/{name}.ogg"])
        return True
