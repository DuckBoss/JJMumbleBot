...
from JJMumbleBot.plugins.extensions.sound_board.utility import (
    sound_board_utility as sbu,
)
...
class Plugin(PluginBase):
    ...
    def cmd_sblist(self, data):
        ...
        if len(internal_list) == 0:
            ...
            gs.gui_service.quick_gui(
                cur_text, text_type="header", box_align="left", user=data_actor["name"]
            )
    ...
    def cmd_sbdownload(self, data):
        data_stripped = BeautifulSoup(
            data.message.strip(), features="html.parser"
        ).get_text()
        ...
        if len(all_data) == 2:
            ...
            if audio_clip:
                dir_utils.remove_file(
                    audio_clip, f"{dir_utils.get_perm_med_dir()}/{self.plugin_name}/"
                )
                gs.gui_service.quick_gui(
                    f"Deleted sound clip : {audio_clip}",
                    text_type="header",
                    box_align="left",
                )
    ...
    def cmd_sbrandom(self, data):
        ...
        if random_sfx == self.last_audio_played:
            random_sfx = random.choice(gather_list)
        self.last_audio_played = random_sfx
        track_obj = TrackInfo(
    ...
    def cmd_sbsearch(self, data):
        ...
        if len(audio_clips) > 0:
            ...
            match_str += "None"
        gs.gui_service.quick_gui(
            match_str, text_type="header", text_align="left", box_align="left"
        )
