...
from JJMumbleBot.plugins.core.web_server.utility.web_utils import (
    ResponseModel,
    StdModel,
    JSONModel,
)
from JJMumbleBot.plugins.core.core_commands.utility import (
    core_commands_utility as cutils,
)
...
class CoreCommandsPluginAPI:
    ...
    async def get_plugin_metadata(self, command: StdModel):
        if len(command.text.strip()) > 0:
            ...
            if plugin_metadata:
                return ResponseModel(
                    200, "success", {"metadata": plugin_metadata}
                ).toDict()
            return ResponseModel(
                400, "error", {"error_message": f"Failed to retrieve metadata file!"}
            ).toDict()
        return ResponseModel(
            400,
            "error",
            {
                "error_message": f"Please make sure you provide an existing plugin to retrieve metadata."
            },
        ).toDict()
    ...
    async def set_plugin_metadata(self, command: JSONModel):
        if len(command.name.strip()) > 0 and command.text:
            ...
            return ResponseModel(
                400, "error", {"error_message": f"Failed to write metadata file!"}
            ).toDict()
        return ResponseModel(
            400,
            "error",
        ...
        if len(command.text.strip()) > 0:
            if cutils.turn_off_plugin(command.text.strip()):
                return ResponseModel(
                    200,
                    "success",
                    {"message": f"Successfully stopped plugin: {command.text.strip()}"},
                ).toDict()
        return ResponseModel(
            400,
            "error",
            {
                "error_message": f"Please make sure you provide an existing, active plugin to stop."
            },
        ).toDict()
        ...
        if len(command.text.strip()) > 0:
            if cutils.turn_on_plugin(command.text.strip()):
                return ResponseModel(
                    200,
                    "success",
                    {"message": f"Successfully started plugin: {command.text.strip()}"},
                ).toDict()
        return ResponseModel(
            400,
            "error",
            {
                "error_message": f"Please make sure you provide an existing, stopped plugin to start."
            },
        ).toDict()
        ...
        if len(command.text.strip()) > 0:
            if cutils.restart_plugin(command.text.strip()):
                return ResponseModel(
                    ...
                    "success",
                    {
                        "message": f"Successfully restarted plugin: {command.text.strip()}"
                    },
                ).toDict()
        return ResponseModel(
            400,
            "error",
            {
                "error_message": f"Please make sure you provide an existing plugin to restart."
            },
        ).toDict()
