...
class CoreRouter:
        ...
        if len(command.text) > 0:
            if command.text[0] == global_settings.cfg[C_MAIN_SETTINGS][P_CMD_TOKEN]:
                ...
                global_settings.core_callbacks.callback(
                    CALLBACK.TEXT_MESSAGE_RECEIVED, text, True
                )
                return ResponseModel(200, "success").toDict()
            return ResponseModel(
                ...
                "error",
                {
                    "error_message": f"The provided command was invalid! Please be sure that your command starts with '{global_settings.cfg[C_MAIN_SETTINGS][P_CMD_TOKEN]}'.\n"
                    f"For Additional assistance, try the {global_settings.cfg[C_MAIN_SETTINGS][P_CMD_TOKEN]}help command."
                },
            ).toDict()
        return ResponseModel(
            500,
            "error",
            {"error_message": f"There was an error processing the provided command."},
        ).toDict()
    ...
    async def last_command(self):
        ...
        return ResponseModel(
            500,
            "error",
            {"error_message": "There was an error retrieving the last command output."},
        ).toDict()
    ...
    async def get_system_info(self):
        return ResponseModel(
            200, "success", {"system": monitor_service.get_system_info()}
        ).toDict()

    @router.get("/api/general/name")
    async def get_name(self):
        return ResponseModel(200, "success", {"name": get_bot_name()}).toDict()
