...
from JJMumbleBot.lib.utils.dir_utils import make_directory, clear_directory, get_temp_med_dir, get_core_plugin_dir, get_perm_med_dir
...
class Plugin(PluginBase):
    def __init__(self):
        ...
        make_directory(f'{gs.cfg[C_MEDIA_SETTINGS][P_PERM_MEDIA_DIR]}/{self.plugin_name}/')
        make_directory(f'{gs.cfg[C_MEDIA_SETTINGS][P_TEMP_MED_DIR]}/{self.plugin_name}/')
    ...
    def util_find_file(self, file_name):
        if not path.exists(f"{get_perm_med_dir()}/sound_board/"):
            log(ERROR, f"The media directory for the 'server_tools' plugin is missing! "
    ...
    def util_find_sb_file(self, file_name):
        if not path.exists(f"{get_perm_med_dir()}/sound_board/"):
            log(ERROR, f"The media directory for the 'sound_board' plugin is missing! "
    ...
    def clbk_user_connected(self, user):
        ...
        if self.metadata.getboolean(C_PLUGIN_SET, P_PLAY_SAME_CLIP_ON_USER_JOIN, fallback=False):
            to_play: str = self.metadata[C_PLUGIN_SET][P_GENERIC_CLIP_TO_PLAY_ON_USER_JOIN]
            ...
            # If the clip is missing, attempt to find the default clip instead.
                ...
                if not du_find_file(self.default_audio_clip,
                    ...
                    log(ERROR, f"The audio clip: {self.default_audio_clip} for the user connection sound could not be found! "
            ...
            if not to_play:
                ...
                if track_obj is None:
                    gs.aud_interface.clear_dni()
                    log(ERROR, f"The audio clip: {to_play} connection sound for the user {user[0]['name']} could not be found.",
                        origin=L_COMMAND,
                        error_type=CMD_PROCESS_ERR, print_mode=PrintMode.VERBOSE_PRINT.value)
    ...
    def cmd_wiki(self, data):
        gs.gui_service.quick_gui(f'<a href="{LINK_WIKI}">Visit the JJMumbleBot Wiki</a>',
