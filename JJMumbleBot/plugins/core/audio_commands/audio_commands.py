...
from JJMumbleBot.plugins.core.audio_commands.utility import (
    audio_commands_utility as ac_utility,
)
...
class Plugin(PluginBase):
    ...
    def cmd_queue(self, data):
        if gs.aud_interface.check_dni_active():
            ...
            for i, item in enumerate(queue_list):
                out_str += item
            gs.gui_service.quick_gui(
                out_str, text_type="header", text_align="left", box_align="left"
            )
            log(
                INFO,
                INFO_DISPLAYED_QUEUE,
                origin=L_COMMAND,
                print_mode=PrintMode.VERBOSE_PRINT.value,
            )
    ...
    def cmd_duckstartdelay(self, data):
        try:
            delay = float(data.message.strip().split(" ", 1)[1])
        except IndexError:
            log(
                INFO,
                f"Displayed current bot audio ducking start delay: {gs.aud_interface.audio_utilities.get_ducking_start_delay()}",
                origin=L_COMMAND,
                print_mode=PrintMode.VERBOSE_PRINT.value,
            )
            gs.gui_service.quick_gui(
                f"Current bot audio ducking start delay: {gs.aud_interface.audio_utilities.get_ducking_start_delay()}",
                text_type="header",
                box_align="left",
                text_align="left",
            )
        ...
        gs.aud_interface.audio_utilities.set_ducking_start_delay(delay)
        log(
            INFO,
            f"Set audio ducking start delay to {delay}",
            origin=L_COMMAND,
            print_mode=PrintMode.VERBOSE_PRINT.value,
        )
        gs.gui_service.quick_gui(
    ...
    def cmd_duckenddelay(self, data):
        try:
            delay = float(data.message.strip().split(" ", 1)[1])
        except IndexError:
            log(
                INFO,
                f"Displayed current bot audio ducking end delay: {gs.aud_interface.audio_utilities.get_ducking_end_delay()}",
                origin=L_COMMAND,
                print_mode=PrintMode.VERBOSE_PRINT.value,
            )
            gs.gui_service.quick_gui(
                f"Current bot audio ducking end delay: {gs.aud_interface.audio_utilities.get_ducking_end_delay()}",
                text_type="header",
                box_align="left",
                text_align="left",
            )
        ...
        gs.aud_interface.audio_utilities.set_ducking_end_delay(delay)
        log(
            INFO,
            f"Set audio ducking end delay to {delay}",
            origin=L_COMMAND,
            print_mode=PrintMode.VERBOSE_PRINT.value,
        )
        gs.gui_service.quick_gui(
