...
from JJMumbleBot.lib.resources.strings import (
    ...
    WARNING,
    L_GENERAL,
    WARN_INVALID_IMG_FORMAT,
    WARN_FIXED_IMG_FORMAT,
    GEN_PROCESS_ERR,
)
...
def format_image(
    ...
    if img_ext.upper() == "PNG":
        ...
        if not quiet:
            log(
                ...
                WARN_IMG_INCORRECT_FORMAT,
                origin=L_GENERAL,
                error_type=WARN_INVALID_IMG_FORMAT,
                print_mode=PrintMode.VERBOSE_PRINT.value,
            )
    # Open images
    img = Image.open(f"{img_dir}/{img_name}.{img_ext}")
    ...
    return format_image_html(
        img_ext=img_ext, byte_arr=img_byte_arr, quiet=quiet, src_only=src_only
    )
...
def download_image_requests(img_url):
    ...
    if r.status_code == 200:
        with open(
            f"{dir_utils.get_temp_med_dir()}/internal/images/_image.{img_ext}", "wb"
        ) as f:
...
def download_image_stream_to_dir(img_url, dir_name, img_name="_image", force_jpg=True):
    ...
    img_ext = img_url.rsplit(".", 1)[1]
    with open(
        f"{dir_utils.get_temp_med_dir()}/{dir_name}/{img_name}.{img_ext}", "wb"
    ) as img_file:
        resp = requests.get(img_url, stream=True)
        for block in resp.iter_content(1024):
            if not block:
    ...
    if img_ext == "png" and force_jpg:
        ...
        dir_utils.remove_file(
            f"{img_name}.png", f"{dir_utils.get_temp_med_dir()}/{dir_name}"
        )
    log(
        INFO,
        INFO_IMG_DOWNLOADED,
...
def download_image_requests_to_dir(
    img_url, dir_name, img_name="_image", force_jpg=True
):
    dir_utils.clear_directory(f"{dir_utils.get_temp_med_dir()}/{dir_name}")
    img_ext = img_url.rsplit(".", 1)[1]
    s = requests.Session()
